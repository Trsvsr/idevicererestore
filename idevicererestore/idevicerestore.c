...
int idevicerestore_start(struct idevicerestore_client_t* client)
{
	...
	if (client->flags & FLAG_RERESTORE) {
		...
		if (!client->manifestPath) {
			...
			client->otamanifest = (char*)client->manifestPath;
		}
		...
		if ((strcmp(device, "iPad2,1") && strcmp(device, "iPad2,4") && strcmp(device, "iPad2,5") && strcmp(device, "iPad3,1") && strcmp(device, "iPad3,4") && strcmp(device, "iPod5,1") && !client->manifestPath)) {
			char* bbfwpath = NULL;
			printf("Downloading baseband firmware for device type %s.\n", device);
